...
#ifdef _LINUX
extern int _func_inb(int port);
extern void _func_outb(int port, int data);
#define func_inb _func_inb;
#define func_outb _func_outb;
#endif
...
void roll_screen() {
	...
	for (i = 1; i < VIDEO_Y_SZ; i++) {
		memcpy(video_buffer + (i - 1) * 80 * 2, video_buffer + i * 80 * 2,
